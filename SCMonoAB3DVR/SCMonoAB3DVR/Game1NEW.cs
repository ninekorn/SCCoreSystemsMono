...
namespace SCMonoAB3DOVR
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        //Microsoft.Xna.Framework.Graphics.Texture2D mirrorTextureD3D = null;
        ...
        protected override void Initialize()
        {
            ...
            renderTarget[0] = new RenderTarget2D(GraphicsDevice,
                                                Width,
                                                Height,
                                                false,
                                                surfaceFormat,
                                                depthFormat);  //, 1, usage, true, 1




            renderTarget[1] = new RenderTarget2D(GraphicsDevice,
                                          Width,
                                          Height,
                                          false,
                                          surfaceFormat,
            ...
            worldMatrix = Matrix.CreateTranslation(0, 0.0f, 0);
            viewMatrix = Camera.View;// Matrix.CreateLookAt(Camera.Position, Camera.Target, Vector3.Up);
            projectionMatrix = Camera.Projection;// Matrix.CreatePerspectiveFieldOfView(fovAngle, aspectRatio, near, far);
        ...
        public void initOculusRift(IntPtr Handle)
        {
            ...
            /*OVR.GetFovTextureSize(sessionPtr, eye, hmdDesc.DefaultEyeFov[eyeIndex], 1.0f);
            Texture2DDescription Texture2DDescription;
            Texture2DDescription = new Texture2DDescription();
            Texture2DDescription.Width = eyeTexture.TextureSize.Width;
            Texture2DDescription.Height = eyeTexture.TextureSize.Height;
        ...
        //#endif
                        ...
                        backBuffer = SharpDX.Direct3D11.Resource.FromSwapChain<SharpDX.Direct3D11.Texture2D>(swapChain, 0);
                        backBufferRenderTargetView = new RenderTargetView(d3dDevice, backBuffer);
                        ...
                        depthBuffer = new SharpDX.Direct3D11.Texture2D(d3dDevice, depthBufferDescription);
                        depthStencilView = new SharpDX.Direct3D11.DepthStencilView(d3dDevice, depthBuffer);
                        depthStencilState = new SharpDX.Direct3D11.DepthStencilState(d3dDevice, depthStencilStateDescription);
                        ...
                        for (int eyeIndex = 0; eyeIndex < 2; eyeIndex++)
                        {
                            ...
                            //Console.WriteLine("x: " + eyeTextures[eyeIndex].HmdToEyeViewOffset.X + " y: " + eyeTextures[eyeIndex].HmdToEyeViewOffset.Y + " z: " + eyeTextures[eyeIndex].HmdToEyeViewOffset.Z);
                            ...
                            //GraphicsDevice.GetNativeDxDeviceAndContext(out dxDevicePtr, out dxContextPtr);

                            result = OVR.CreateTextureSwapChainDX(sessionPtr, d3dDevice.NativePointer, ref textureSwapChainDesc, out textureSwapChainPtr);
                            //result = OVR.CreateTextureSwapChainDX(sessionPtr, dxDevicePtr, ref textureSwapChainDesc, out textureSwapChainPtr);
                            ...
                            {
                                Console.WriteLine("PTR ZERO");
                            }
                            else
                            {
                                Console.WriteLine("PTR: " + textureSwapChainPtr);
                            }
                            ...
                            //Console.WriteLine(textureSwapChainBufferCount);
                            ...
                            //eyeTexture.RenderTargetViewsXNA = new Microsoft.Xna.Framework.Graphics.RenderTarget2D[textureSwapChainBufferCount];
                            ...
                            for (int textureIndex = 0; textureIndex < textureSwapChainBufferCount; textureIndex++)
                            {
                                ...
                                //eyeTexture.TexturesXNA[textureIndex] = new Microsoft.Xna.Framework.Graphics.Texture2D(GraphicsDevice, eyeTexture.TextureSize.Width, eyeTexture.TextureSize.Height);
                                // Create a render target view for the current Texture2D.

                                eyeTexture.RenderTargetViewsSHARPDX[textureIndex] = new RenderTargetView(d3dDevice, eyeTexture.TexturesSHARPDX[textureIndex]);
                                /*eyeTexture.RenderTargetViewsXNA[textureIndex] = new RenderTarget2D(GraphicsDevice,
                                                                                                    eyeTexture.TextureSize.Width,
                                                                                                    eyeTexture.TextureSize.Height,
                                                                                                    true,
                                                                                                    surfaceFormat,
                                                                                                    depthFormat); //, 1, usage, true, 1*/
                            }
                        ...
                        result = OVR.CreateMirrorTextureDX(sessionPtr, d3dDevice.NativePointer, ref mirrorTextureDescription, out mirrorTexturePtr);
                        //result = OVR.CreateMirrorTextureDX(sessionPtr, dxDevicePtr, ref mirrorTextureDescription, out mirrorTexturePtr);
                        ...
                        //mirrorTextureD3D = new Microsoft.Xna.Framework.Graphics.Texture2D(GraphicsDevice, Width, Height);// eyeTexture.TextureSize.Width, eyeTexture.TextureSize.Height);
            ...
            try
            {
                ...
                */
                ...
                //mirrorTextureD3D.GetSharedHandle();

                //GraphicsDevice.SetRenderTarget(eyeTexture.RenderTargetViewsXNA[textureIndex]);
        ...
        public void renderOculus(GameTime gameTime)
        {

            GraphicsDevice.SetRenderTarget(renderTarget[0]);// eyeTexture.RenderTargetViewsXNA[textureIndex]);// renderTarget[eyeIndex]);
            GraphicsDevice.Clear(Microsoft.Xna.Framework.Color.CornflowerBlue); //new Microsoft.Xna.Framework.Color(130, 180, 255)
            ...
            {               
                ...
                for (int eyeIndex = 0; eyeIndex < 2; eyeIndex++)
                {
                    ...
                    //eyeTexture.RenderTargetViewsXNA[textureIndex].SetNativeDxResource(eyeTexture.SwapTextureSet.TextureSwapChainPtr);
                    //eyeTexture.RenderTargetViewsXNA[textureIndex].SetNativeDxResource(eyeTexture.SwapTextureSet.TextureSwapChainPtr);

                    //SwapChainRenderTarget test = new SwapChainRenderTarget(GraphicsDevice, currentWindow.Window.Handle, GraphicsDevice.PresentationParameters.BackBufferWidth, GraphicsDevice.PresentationParameters.BackBufferHeight);
                    //test.re

                    //base.BeginDraw();
                    //GraphicsDevice.SetRenderTarget(eyeTexture.RenderTargetViewsXNA[textureIndex]);// renderTarget[eyeIndex]);
                    //GraphicsDevice.Clear(Microsoft.Xna.Framework.Color.CornflowerBlue); //new Microsoft.Xna.Framework.Color(130, 180, 255)

                    //SetBackBuffer(eyeIndex, mirrorTextureD3D);
                    //base.EndDraw();
                    //GraphicsDevice.BlendState = Microsoft.Xna.Framework.Graphics.BlendState.Opaque;
                    //GraphicsDevice.DepthStencilState = Microsoft.Xna.Framework.Graphics.DepthStencilState.Default;
                    ...
                    */
                    ...
                    SharpDX.Vector3 viewPosition = new SharpDX.Vector3(0,1,0) - eyePoses[eyeIndex].Position.ToVector3();

                    SharpDX.Matrix world = SharpDX.Matrix.Identity;// SharpDX.Matrix.Scaling(0.1f) * Matrix.RotationX(timeSinceStart / 10f) * Matrix.RotationY(timeSinceStart * 2 / 10f) * Matrix.RotationZ(timeSinceStart * 3 / 10f);
                    SharpDX.Matrix viewMatrixx = SharpDX.Matrix.LookAtLH(viewPosition, viewPosition + lookAt, lookUp);

                    SharpDX.Matrix projectionMatrixx = OVR.Matrix4f_Projection(eyeTexture.FieldOfView, 0.1f, 100.0f, ProjectionModifier.LeftHanded).ToMatrix();
                    projectionMatrixx.Transpose();

                    //SharpDX.Matrix worldViewProjection = world * viewMatrix * projectionMatrix;
                    //worldViewProjection.Transpose();

                    if (effectLoaded == 1)
                    {
                        Matrix view;
                        ConvertMatrix(viewMatrixx ,out view);
                        Matrix proj;
                        ConvertMatrix(projectionMatrixx, out proj);


                        effect.CurrentTechnique = effect.Techniques["Instancing"];
                        effect.Parameters["WVP"].SetValue(viewMatrix * projectionMatrix);// view * proj);
                    ...
                    /*using (var screenTexture2D = eyeTexture.RenderTargetViewsSHARPDX[textureIndex].Resource)// eyeTexture.TexturesSHARPDX[textureIndex].QueryInterface<SharpDX.Direct3D11.Texture2D>())// _screenResource.QueryInterface<Texture2D>())
                    {
                        immediateContext.CopyResource(screenTexture2D, _texture2D);
                    }
                    ...
                    int memoryBitmapStride = columns * 4;
                    Bitmap someBitmap = new Bitmap(columns, rows, memoryBitmapStride, PixelFormat.Format32bppArgb, sourcePtr);

                    someBitmap.Save(@"C:\Users\ninekorn\Desktop\monoScreen\" + counter + ".png");
                    DeleteObject(sourcePtr);

                    someBitmap.Dispose();
                    immediateContext.UnmapSubresource(_texture2D, 0);*/
                    ...
                    //eyeTexture.TexturesXNA[eyeIndex].GetSharedHandle()
                    //base.Draw(gameTime);
                    //
                    //eyeTexture.RenderTargetViewsXNA[textureIndex].tex

                    /*var width = eyeTexture.RenderTargetViewsXNA[textureIndex].Width;
                    var height = eyeTexture.RenderTargetViewsXNA[textureIndex].Height;
                    Microsoft.Xna.Framework.Color[] col = new Microsoft.Xna.Framework.Color[width * height];

                    eyeTexture.RenderTargetViewsXNA[textureIndex].GetData<Microsoft.Xna.Framework.Color>(col,0, col.Length);

                    Texture2D tex = new Texture2D(GraphicsDevice,width,height);

                    tex.SetData<Microsoft.Xna.Framework.Color>(col);
                    IntPtr handleTo = tex.GetSharedHandle();*/




                    //eyeTexture.RenderTargetViewsXNA[textureIndex].GetSharedHandle()
                    //result = OVR.CommitTextureSwapChain(sessionPtr, eyeTexture.SwapTextureSet.TextureSwapChainPtr);// eyeTexture.RenderTargetViewsXNA[textureIndex].GetSharedHandle());// eyeTexture.SwapTextureSet.TextureSwapChainPtr);// eyeTexture.SwapTextureSet.TextureSwapChainPtr); //eyeTexture.RenderTargetViewsXNA[textureIndex].GetNativeDxResource()
                ...
                using (var screenTexture2D = backBuffer.QueryInterface<SharpDX.Direct3D11.Texture2D>())// eyeTexture.RenderTargetViewsSHARPDX[textureIndex].Resource)// eyeTexture.TexturesSHARPDX[textureIndex].QueryInterface<SharpDX.Direct3D11.Texture2D>())// _screenResource.QueryInterface<Texture2D>())
                {
                    immediateContext.CopyResource(screenTexture2D, _texture2D);
                }
                var dataBox = device.ImmediateContext.MapSubresource(_texture2D, 0, SharpDX.Direct3D11.MapMode.Read, SharpDX.Direct3D11.MapFlags.None);
            ...
            //eyeTextures[0].RenderTargetViewsXNA[0] = (RenderTarget2D)mirrorTextureD3D;

            //SetBackBuffer(0, eyeTextures[0].RenderTargetViewsXNA[0]);
            /*var width = mirrorTextureD3D.Width;// eyeTextures[0].RenderTargetViewsXNA[0].Width;
            var height = mirrorTextureD3D.Height;//eyeTextures[0].RenderTargetViewsXNA[0].Height;
            Microsoft.Xna.Framework.Color[] col = new Microsoft.Xna.Framework.Color[width * height];

            mirrorTextureD3D.GetData<Microsoft.Xna.Framework.Color>(col, 0, col.Length);
            //eyeTextures[0].RenderTargetViewsXNA[0].GetData<Microsoft.Xna.Framework.Color>(col, 0, col.Length);

            //Texture2D tex = new Texture2D(GraphicsDevice, width, height);

            eyeTextures[0].RenderTargetViewsXNA[0].SetData<Microsoft.Xna.Framework.Color>(col);
            //IntPtr handleTo = tex.GetSharedHandle();*/
            ...
            {
                result = eyeTextures[i].SwapTextureSet.Commit();
                WriteErrorDetails(OVR, result, "Failed to commit the swap chain texture.");
            }*/
        ...
        public void SetBackBuffer(int eyeIndex, RenderTarget2D renderTarg)
        {
            var width = renderTarg.Width;
            var height = renderTarg.Height;
            //Microsoft.Xna.Framework.Color[] data = new Microsoft.Xna.Framework.Color[width * height];
            /*int[] data = new int[width * height * 4];

            graphics.GraphicsDevice.GetBackBufferData<>


            renderTarg.GetData<int>(data, 0, data.Length); //Microsoft.Xna.Framework.Color

            byte[] someArray = new byte[data.Length];

            for (int i = 0; i < someArray.Length; i++)
            {
                someArray[i] = (byte)data[i];
            }*/
            //for (int i = 0; i < someArray.Length/4; i++)
            //{
            //    someArray[i * 4 + 0] = data[i * 4 + 0].A;
            //    someArray[i * 4 + 1] = data[i * 4 + 1].R;
             //   someArray[i * 4 + 2] = data[i * 4 + 2].G;
            //    someArray[i * 4 + 3] = data[i * 4 + 3].B;
            //}
            ...
            /*var ptr = Marshal.UnsafeAddrOfPinnedArrayElement(someArray, 0);

            int memoryBitmapStride = width * 4;
            Bitmap someBitmap = new Bitmap(width, height, memoryBitmapStride, PixelFormat.Format32bppArgb, ptr);
        ...
        void DrawEyeViewIntoBackbuffer(int eye)
        {
            ...
            spriteBatch.Begin();
            spriteBatch.Draw(renderTarget[eye], new Microsoft.Xna.Framework.Rectangle(offset, 0, width, height), Microsoft.Xna.Framework.Color.White);
            spriteBatch.End();
        }
        ...
        protected override void LoadContent()
        {
            ...
            //Console.WriteLine(effectLoaded);
        ...
        private void GenerateInstanceInformation(GraphicsDevice device, Int32 count)
        {
            ...
            for (int x = 0; x < instx; x++)
            {
                for (int y = 0; y < insty; y++)
                {
                    for (int z = 0; z < instz; z++)
                    {
                        ...
                        instances[index].World = new Vector4(x * 2, (y * 2) + 50, z * 2, 1);
            ...
            /*for (int i = 0; i < count; i++)
            {
                ...
                instances[i].World = new Vector4(-rnd.Next(10), -rnd.Next(10), -rnd.Next(10), 1);
